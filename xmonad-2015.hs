...
import           XMonad.Layout.NoBorders          (lessBorders, noBorders,
                                                   smartBorders)
...
myTabbed =
  renamed [R.Replace "＋"] $ noBorders $ tabbedBottom shrinkText myTabTheme
...
myLayoutHook =
  ...
  onWorkspace "9" myLayoutTerm myLayoutDefault
...
myManageHook =
  ...
  insertPosition Above Newer -- default above newer is ok.
   <+>
  composeOne
    [fmap not isDialog -?> doF avoidMaster, return True -?> doF W.swapDown] <+>
  composeOne
    [isFullscreen -?> doFullFloat, isDialog -?> doCenterFloat <+> doF W.swapUp]
